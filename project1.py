...
def create():
    ...
    query = "use school"
    cur.execute(query)
    query = "create table students(grno int primary key, name char(20), classs int(5), section char(5), grade char(5))"
    cur.execute(query)
...
def search():
    ...
    ans = 'y'
    while ans.lower() == 'y':
        no = int(input("ENTER STUDENT TO SEARCH :"))
...
def update():
    ...
    while ans.lower() == 'y':
        ...
        if result != None:
            ...
            for row in result:
                print(row)
                an = input("What do you want to update?(Enter 1 - Name\n2-Class\n3-Grade):")
                if an == '1':
